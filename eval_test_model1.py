...
if '.txt' in args.dataset:
    dataset_eval = utils.PathDataseth5seqtxt(hdf5_file=args.dataset.replace("train", "eval"), seq_len=args.seqences_len, path_length=1)
else:
...
max_number = -1
record_name = ''
#model_name = ["model_epoch34_H1_0.671_MRR_tensor.pt"]
for name in model_name:
    if '.pt' in name and 'H1_' in name and '_MRR' in name:
        ...
        # else:
        #     model_name = 'model_epoch13.pt'
        # model_name = name
        print(name)

        #model_name = "model_epoch105_H1_0.32095_MRR_tensor.pt"# Ice_obj3_trans_layers2_seq10_sub_03_txt
        print(name, args_eval)
        meta_file = os.path.join(args_eval.save_folder, 'metadata.pkl')
        model_file = os.path.join(args_eval.save_folder, name)
        ...
        epoch_num = name.split('epoch')[1].split('_')[0]
        ...
        with torch.no_grad():
            ...
            for batch_idx, data_batch in enumerate(eval_loader):
                ...
                #print('obj_state', obj_state.mean(3).mean(2))
            ...
            #print(next_state_flat.shape, pred_state_flat.shape)

...
def write_json(img_ls, tr_masks, pr_masks):
    ...
    fw = open("tr_label_roi.json", 'w')
    fw.write(json.dumps(dataset,cls=MyEncoder,indent=4))
    fw = open("pr_label_roi.json", 'w')
...
tr_masks = []
with torch.no_grad():
    for i in range(len(img_ls)):#(len(img_ls)):
        # read true img mask
...
coco_eval_mask("tr_label_roi.json", "pr_label_roi.json")
